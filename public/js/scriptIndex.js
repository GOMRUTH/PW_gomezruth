document.addEventListener('DOMContentLoaded', function() {
    ...
    const collageProductos = document.getElementById('collage-productos');

    // Función para generar los botones de productos
    function generarBotonesProductos() {
        productos.forEach(producto => {
            const botonProducto = document.createElement('button');
            botonProducto.classList.add('boton-detalle-carrito');
            botonProducto.innerHTML = `<img src="${producto.imagen}" alt="Producto ${producto.id}">`;
            botonProducto.addEventListener('click', function() {
                window.location.href = `detalleProducto.html?id=${producto.id}`;
            });
            collageProductos.appendChild(botonProducto);
        });
    }

    // Llamar a la función para generar los botones de productos al cargar la página
